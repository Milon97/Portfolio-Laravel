        ...
        <div class="col-12">
            <div class="page-title-box">
                <div class="page-title-right">
                    <a href="{{ route('logo.create') }}" class="btn btn-primary waves-effect waves-light btn-sm rounded-pill">Create</a>
                </div>
                    ...
                    <table id="datatable-buttons" class="table table-striped dt-responsive nowrap w-100">
                        ...
                        <tbody>
                            ...
                            <tr style="vertical-align: middle;">
                                ...
                                <td><img src="{{ asset($value->image) }}" class="avatar-lg rounded-4"></td>
                                ...
                                <td>
                                    <div class="button-list">
                                        @if($value->status == 1)
                                        <form method="post" action="{{ route('logo.inactive') }}" class="d-inline">
                                            @csrf
                                        ...
                                        @else
                                        <form method="post" action="{{ route('logo.active') }}" class="d-inline">
                                            @csrf
                                            <input type="hidden" value="{{$value->id}}" name="hidden_id" />
                                        ...
                                        <a href="{{ route('logo.edit',$value->id) }}" class="btn btn-xs btn-primary waves-effect waves-light"><i class="fe-edit-1"></i></a>

                                        <form method="post" action="{{ route('logo.destroy') }}" class="d-inline">
                                            @csrf
                                            <input type="hidden" value="{{$value->id}}" name="hidden_id" />
